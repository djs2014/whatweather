...
function isDelayedFor(timevalue, minutesDelayed) {	
	if (timevalue == null || minutesDelayed <= 0) { return false;}
...
function getDateTimeString(moment) {
	if (moment == null || moment == 0) {return "";}
	var date = Calendar.info(moment, Time.FORMAT_SHORT);
	return date.day.format("%02d")+"-"+date.month.format("%02d")+"-"+date.year.format("%d")+" " + date.hour.format("%02d") +":" + date.min.format("%02d")+":" + date.sec.format("%02d");
}
