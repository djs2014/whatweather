...
app.http('weather', {
    ...
    handler: async (request, context) => {
        ...
        try {
            ...
            let resBody = "";
            let testScenario = searchParams.get("testScenario");
            // TODO
            // if (testScenario) {
            //     try {
            //         context.log("use test data: " + testScenario);
            //         // 1: rain
            //         // 2: alerts
            //         resBody = await fs.readFile('./data/owm_sample_' + testScenario + '.json', { encoding: 'utf8' });
            //         res.writeHead(200, { 'Content-Type': 'application/json' });
            //         res.end(JSON.stringify(owm.convertTestdata(testScenario, request.query.appid, request.query.lat, request.query.lon,
